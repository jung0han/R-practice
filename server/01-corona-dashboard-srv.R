...
dash_filters <- reactive({
  ...
  out <- val_tbl() %>%
    filter(type %in% input$dash_status, country %in% input$dash_state)
...
ay_plot_prep <- reactive({
  ...
  status <- if (length(input$dash_status) == 3) "All" else paste0(input$dash_status, collapse=", ")
  states <- if (length(input$dash_state) == 191) "All" else paste0(input$dash_state, collapse=", ")
  
  subtitle <- paste0(
    "Status: ", status, "; States: ", states, "; Excluding Claims Below: "
  )
